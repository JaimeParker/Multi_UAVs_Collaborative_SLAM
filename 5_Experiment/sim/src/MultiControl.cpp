...
void Offboard(ros::ServiceClient &set_mode_client, mavros_msgs::SetMode &offb_set_mode,
              ...
              ros::Rate &rate, mavros_msgs::State &state,
    ...
    while (ros::ok()) {
        if (state.mode != "OFFBOARD" &&
            ...
            if (!state.armed &&
                (ros::Time::now() - last_request > ros::Duration(5.0))) {
                if (state.mode != "OFFBOARD") ROS_INFO("Off board mode was shut unexpectedly");
        ...
        geometry_msgs::Point curr,aim;
        curr = current_position.pose.position;
    ...
    for(;;){
        ...
        switch (c) {
            case KEYCODE_T:
                ...
                Offboard(set_mode_client0, offboard_mode_set, arm_cmd,
                         arming_client0, local_pos_pub0, pose_uav0,
                         rate, current_state, current_pose);
                break;
            case KEYCODE_L:
                ROS_INFO("Offboard for UAV1");
                Offboard(set_mode_client1, offboard_mode_set1, arm_cmd1,
                         arming_client1, local_pos_pub1, pose_uav1,
                         rate, current_state1, current_pose1);
                break;
            case KEYCODE_H:
		...
		flag = true;
            ...
            case KEYCODE_W:
                ...
                ROS_INFO("add X Speed for UAVs: %f", twist.linear.x);
                flag  = true;
                break;
            case KEYCODE_S:
                twist.linear.x -= ACCEL;
                ...
                ROS_INFO("dec X Speed for UAVs: %f", twist.linear.x);
                flag  = true;
                break;
            case KEYCODE_A:
                ...
                ROS_INFO("add Y Speed for UAV2: %f", twist.linear.y);
                flag  = true;
                break;
            case KEYCODE_D:
                twist.linear.y -= ACCEL;
            ...
            case KEYCODE_UP:
                twist.linear.z += ACCEL;
                if (twist.linear.z > MAX_SPEED) twist.linear.z = MAX_SPEED;
                twist1.linear.z += ACCEL;
                if (twist1.linear.z > MAX_SPEED) twist1.linear.z = MAX_SPEED;
                ROS_INFO("up Speed for UAVs: %f", twist1.linear.z);
                flag  = true;
                break;
            case KEYCODE_DOWN:
                twist1.linear.z -= ACCEL;
                if (twist1.linear.z < MIN_SPEED) twist1.linear.z = MIN_SPEED;
                twist.linear.z -= ACCEL;
                if (twist.linear.z < MIN_SPEED) twist.linear.z = MIN_SPEED;
                ROS_INFO("down Speed for UAVs: %f", twist1.linear.z);
                flag  = true;
                break;
            case KEYCODE_LEFT:
                twist.angular.z += ACCEL;
                if (twist.angular.z > MAX_SPEED) twist.angular.z = MAX_SPEED;
                twist1.angular.z += ACCEL;
                if (twist1.angular.z > MAX_SPEED) twist1.angular.z = MAX_SPEED;
                ROS_INFO("add angular speed for UAVs: %f", twist1.angular.z);
                flag  = true;
                break;
            case KEYCODE_RIGHT:
                twist1.angular.y -= ACCEL;
                if (twist1.angular.y < MIN_SPEED) twist1.angular.y = MIN_SPEED;
                twist.angular.y -= ACCEL;
                if (twist.angular.y < MIN_SPEED) twist.angular.y = MIN_SPEED;
                ROS_INFO("dec angular Speed for UAV1: %f", twist1.angular.z);
                flag  = true;
                break;
