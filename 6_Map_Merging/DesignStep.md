...
设有四张同一场景不同位置的图片，1-2-3-4

现得到1-2之间和3-4之间的转换矩阵$T_{12},T_{34}$，则有了两套坐标转换关系；并且根据三角测量，得到了两组的地图点相对于各自参考图片的位置$P_{12},P_{34}$；

由于是相同场景，假设1-2-3-4需要完成地图融合（已满足外观相似条件，其他条件暂不考虑）；

对1-3进行特征点提取与匹配，得到1-3的转换矩阵$T_{13}$，则可以将3中的地图点转换到1坐标系下：
