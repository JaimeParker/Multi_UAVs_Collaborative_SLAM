...
mavros_msgs::State current_state2; /* NOLINT */
...
int main(int argc, char **argv){
    ...
    ros::Subscriber state_sub2 = n2.subscribe<mavros_msgs::State>
            ("uav2/mavros/state", 10, state_cb2);
    ...
    while(ros::ok()){
        ...
        if (current_state.mode != "OFFBOARD" && ros::Time::now() - last_request > ros::Duration(5.0)){
            ...
            ROS_INFO("uav0---Offboard mode enabling...");
            if (offboard_mode_set.response.mode_sent){
                ROS_INFO("uav0---Offboard enabled successfully!");
            }
            last_request = ros::Time::now();
        }
        if (current_state1.mode != "OFFBOARD" && ros::Time::now() - last_request > ros::Duration(5.0)){
            set_mode_client1.call(offboard_mode_set1);
            ROS_INFO("uav1---Offboard mode enabling...");
            if (offboard_mode_set1.response.mode_sent){
                ROS_INFO("uav1---Offboard enabled successfully!");
            }
            last_request = ros::Time::now();
        }
        if (current_state2.mode != "OFFBOARD" && ros::Time::now() - last_request > ros::Duration(5.0)){
            set_mode_client2.call(offboard_mode_set2);
            ROS_INFO("uav2---Offboard mode enabling...");
            if (offboard_mode_set2.response.mode_sent){
                ROS_INFO("uav2---Offboard enabled successfully!");
            }
            last_request = ros::Time::now();
        }
